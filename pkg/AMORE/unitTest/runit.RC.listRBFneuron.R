...
test.gListRBFneuron.buildAndAppend <- function() {
	ln <- gListRBFneuron$new()
	ln$buildAndAppend(ID=list(1,2,3,4,5), WIDTH=list(1.1,3.4,5.4,9.8,5.6), ALTITUDE=list(1.9,8.4,8.7,3.5,7.6), FROM=list(1:3,1:3,1:3,1:3,1:3), WEIGHT=list(11:13,21:23,31:33,41:43,51:53))
	checkTrue(ln$is.regular())
	checkTrue(ln$validate())
	rm(ln)
	ln <- gListRBFneuron$new()
	ln$buildAndAppend(ID=list(1,2,3,4,5), WIDTH=list(1.1,3.4,5.4,9.8,5.6), ALTITUDE=list(1.9,8.4,8.7,3.5,7.6), FROM=list(1:3,4:6,7:9,10:12,13:15), WEIGHT=list(11:13,21:23,31:33,41:43,51:53))
	checkTrue(!ln$is.regular())
